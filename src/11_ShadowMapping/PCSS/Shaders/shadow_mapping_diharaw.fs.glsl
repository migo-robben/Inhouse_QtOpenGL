...
#define PCF 1
#define PCSS 2
#define BLOCKERSEARCHSAMPLES 100 //25 32 64 100 128
#define PCFSAMPLES 128
...
float SearchRegionRadiusUV(float z) {
    // z is distance from shading point to light from light view space, not fragment pos
    float blockerSearchScale = 1.0f;

    return blockerSearchScale * LIGHTSIZE * (z - NEAR) / z;
}
...
void FineBlocker(out float accum_blocker_depth,
    ...
    num_blockers        = 0.0;
    max_blockers        = float(BLOCKERSEARCHSAMPLES);
    ...
    for (int i = 0; i < BLOCKERSEARCHSAMPLES; ++i)
    {
        ...
        if (BLOCKERSEARCHSAMPLES == 25)
            offset = Poisson25[i];
        else if (BLOCKERSEARCHSAMPLES == 32)
            offset = Poisson32[i];
        else if (BLOCKERSEARCHSAMPLES == 64)
            offset = Poisson64[i];
        else if (BLOCKERSEARCHSAMPLES == 100)
            offset = Poisson100[i];
        else if (BLOCKERSEARCHSAMPLES == 128)
            offset = Poisson128[i];
...
float pcfPoissonFilter(vec2 uv, float z0, float bias, float filter_radius_uv)
{
    ...
    for (int i = 0; i < PCFSAMPLES; ++i)
    {
        ...
        if (PCFSAMPLES == 25)
            offset = Poisson25[i];
        else if (PCFSAMPLES == 32)
            offset = Poisson32[i];
        else if (PCFSAMPLES == 64)
            offset = Poisson64[i];
        else if (PCFSAMPLES == 100)
            offset = Poisson100[i];
        else if (PCFSAMPLES == 128)
            offset = Poisson128[i];
...
float PcssFilter(vec2 uv, float z, float bias, float z_vs, out float p_r, out vec3 debug_color) {
    // ------------------------
    // STEP 1: blocker search
    // ------------------------
    float accum_blocker_depth, num_blockers, max_blockers;
    float search_region_radius_uv = SearchRegionRadiusUV(z_vs);
    FineBlocker(accum_blocker_depth, num_blockers, max_blockers, uv, z, bias, search_region_radius_uv);
    ...
    // STEP 3: filtering
    // ------------------------
    return pcfPoissonFilter(uv, z, bias, filter_radius);
}

float ShadowCalculation(vec4 fragPosLightSpace, vec4 posLightSpace, out float search_radius, out float p_r, out vec3 debug_color) {
    ...
    float shadow = 0.0;
    shadow = PcssFilter(projCoords.xy, currentDepth, bias, -(pos_vs.z), p_r, debug_color);
...
void main() {
    ...
    float shadow = ShadowCalculation(fs_in.FragPosLightSpace, fs_in.PosLightSpace, r, p_r, num_blockers_color);
