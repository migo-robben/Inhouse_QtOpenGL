...
MainWidget::MainWidget(QWidget *parent)
    ...
    QVector3D cameraPos(0.0, 0.0, 10.0);
...
void MainWidget::initShaders() {
    if (!SHADER(0)->addShaderFromSourceFile(QOpenGLShader::Vertex, "src/11_ShadowMapping/PCSS/Shaders/shadow_mapping_depth.vs.glsl"))
        close();
    if (!SHADER(0)->addShaderFromSourceFile(QOpenGLShader::Fragment, "src/11_ShadowMapping/PCSS/Shaders/shadow_mapping_depth.fs.glsl"))
        close();
    if (!SHADER(0)->link())
        close();
    if (!SHADER(0)->bind())
    ...
    if (!SHADER(1)->addShaderFromSourceFile(QOpenGLShader::Vertex, "src/11_ShadowMapping/PCSS/Shaders/debug_quad.vs.glsl"))
        close();
    if (!SHADER(1)->addShaderFromSourceFile(QOpenGLShader::Fragment, "src/11_ShadowMapping/PCSS/Shaders/debug_quad.fs.glsl"))
        close();
    if (!SHADER(1)->link())
        close();
    if (!SHADER(1)->bind())
    ...
    if (!SHADER(2)->addShaderFromSourceFile(QOpenGLShader::Vertex, "src/11_ShadowMapping/PCSS/Shaders/shadow_mapping.vs.glsl"))
        close();
    bool diharawMethod = false;
    if (diharawMethod)
    {
        if (!SHADER(2)->addShaderFromSourceFile(QOpenGLShader::Fragment, "src/11_ShadowMapping/PCSS/Shaders/shadow_mapping_diharaw.fs.glsl"))
            close();
    }
    else{
        if (!SHADER(2)->addShaderFromSourceFile(QOpenGLShader::Fragment, "src/11_ShadowMapping/PCSS/Shaders/shadow_mapping.fs.glsl"))
            close();
    }
    if (!SHADER(2)->link())
        close();
    ...
    if (!SHADER(3)->addShaderFromSourceFile(QOpenGLShader::Vertex, "src/11_ShadowMapping/PCSS/Shaders/areaLight.vs.glsl"))
        close();
    if (!SHADER(3)->addShaderFromSourceFile(QOpenGLShader::Fragment, "src/11_ShadowMapping/PCSS/Shaders/areaLight.fs.glsl"))
        close();
    if (!SHADER(3)->link())
        close();
    if (!SHADER(3)->bind())
...
void MainWidget::initGeometry() {
    ...
    light_geometry = new FloorGeometry;
    light_geometry->initGeometry();
    light_geometry->setupAttributePointer(SHADER(3));
...
void MainWidget::renderScene() {
    ...
    SHADER(2)->setUniformValue("MODE", 2);
    ...
    // box 3
    model.setToIdentity();
    model.translate(-1.0, 3.48, -2.5);
    model.scale(0.5, 5.0, 0.5);
    SHADER(2)->setUniformValue("model", model);
    cube_geometry->drawGeometry(SHADER(2));

    // floor
    model.setToIdentity();
    model.translate(2.5, -0.025, 0);
...
void MainWidget::renderLight() {
    ...
    QVector3D dir = QVector3D(0, 0, 0) - lightPos;
    dir = dir.normalized();
    QVector3D axis = QVector3D::crossProduct(QVector3D(0.0, 1.0, 0.0), dir);
    axis = axis.normalized();
    float angle = QVector3D::dotProduct(dir, QVector3D(0.0, 1.0, 0.0));
    angle = qRadiansToDegrees(qAcos(angle));

    model.rotate(QQuaternion::fromAxisAndAngle(axis, angle));
    model.scale(0.5);

    SHADER(3)->bind();
    SHADER(3)->setUniformValue("model", model);
    SHADER(3)->setUniformValue("view", camera->getCameraView());
    SHADER(3)->setUniformValue("projection", camera->getCameraProjection());

    light_geometry->drawGeometry(
            SHADER(3));
}
...
void MainWidget::configLight() {
    ...
    qreal aspect = qreal(width()) / qreal(height() ? height() : 1);
    lightProjection.perspective(60.0f, aspect, near_plane, far_plane);
