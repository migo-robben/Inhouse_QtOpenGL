...
void MainWidget::initShaders() {
    if (!program->addShaderFromSourceFile(QOpenGLShader::Vertex, "src\\4_OpenGLCamera\\Cube.vs.glsl"))
        close();

    if (!program->addShaderFromSourceFile(QOpenGLShader::Fragment, "src\\4_OpenGLCamera\\Cube.fs.glsl"))
        close();
